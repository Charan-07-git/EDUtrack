...
const tiles = [
  ...
  },
  {
    ...
    label: 'Calendar & Calculator',
    sub: 'View schedule & plan ahead',
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const dept = localStorage.getItem("edutrack_department") || "CSE";
    setSetupLabel(year ? `Year ${year} | Semester ${sem} | ${dept}` : 'Set up your academic year');
    if (!year || !sem || !localStorage.getItem("edutrack_department")) {
        ...
        {tiles.map((t) => (
          ...
          >
            ...
            >
              ...
              {t.value && (
                <p className="relative z-10 text-white text-4xl lg:text-5xl font-extrabold">{t.value}</p>
              )}
            </Link>
          </motion.div>
        ))}
